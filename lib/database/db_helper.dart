...
class DbHelper {
  ...
  Future<void> deleteItem(int itemId) async {
    ...
    final outfitIds = await db.rawQuery('SELECT outfitId FROM outfitItems WHERE itemId=$itemId');

    await db.rawDelete('DELETE FROM outfitItems WHERE itemId=$itemId');

    // Deletes any outfits if the last item was deleted from it
  ...
  Future<void> saveOutfit(Outfit outfit) async {
    ...
    for (int itemId in outfit.itemsInOutfit) {
      await db.rawInsert(
          'INSERT OR REPLACE INTO outfitItems (itemId, outfitId) VALUES ($itemId, ${outfit.id})');
    }
  }
  ...
  Future<List<Outfit>> getOutfits() async {
    ...
    final List<Outfit> outfits = List.generate(maps.length, (i) {
